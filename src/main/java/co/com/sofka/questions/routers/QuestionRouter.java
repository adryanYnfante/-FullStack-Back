...
public class QuestionRouter {
    ...
    public RouterFunction<ServerResponse> updateQuestion(UpdateUseCase updateUseCase) {
        return route(POST("/update").and(accept(MediaType.APPLICATION_JSON)),
                request -> request.bodyToMono(QuestionDTO.class)
                        .flatMap(addQuestionDTO -> updateUseCase.apply(addQuestionDTO)
                                .flatMap(result -> ServerResponse.ok()
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .bodyValue(result))
                        )
    ...
    public RouterFunction<ServerResponse> getQuestionPaged(ListUseCase listUseCase) {
        return route(GET("/pagination/{page}"),
                request -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(listUseCase.getPage(Integer.parseInt(request.pathVariable("page"))),
                                QuestionDTO.class))
